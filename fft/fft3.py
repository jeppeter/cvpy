...
def ShowFFT3(infile):
	img = cv2.imread(infile,0) #直接读为灰度图像
	f = np.fft.fft2(img)
	...
	outfile = '%s.inverse.bmp'%(infile)
	cv2.imwrite(outfile,img_back)
